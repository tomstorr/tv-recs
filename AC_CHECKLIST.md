...
- **AUTH-5** ✓ Drive errors classified as `kind="auth"` (`js/drive.js → classify`) propagate to `state.internal.authError`. `js/app.js → render()` then clears the stored token (`auth.clearToken()`) and renders `ui.renderReconnect`. In-memory `state.internal.data` is preserved across the auth error (only on user-initiated sign-out is `state.reset()` called). **Enhancement (post-spec):** before classifying as auth error, `js/drive.js → fetchWithRefresh` attempts one silent token refresh via `auth.requestTokenSilently` (GIS `requestAccessToken({ prompt: '' })`). For users still signed into Google in the browser, the new access token is minted invisibly and the original request is retried — making the once-an-hour token expiry transparent in normal use. If silent refresh fails (revoked grant, signed out of Google, etc.) the flow falls through to the standard Reconnect screen as specified.
