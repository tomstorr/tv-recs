...
- **ACT-4**: Dismiss invokes `mutators.addManualToWatched({title, tmdbId}, "dismissed", null)`. Same mutator. The `"dismissed"` feedback (with `watchedAt = null`) is exclusion-only: SKILL.md keeps the show out of future picks but does not feed any positive, negative, or ambiguous signal from it.
