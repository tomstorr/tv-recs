...
- **FEEDBACK-4**: Tapping Dismiss invokes `mutators.dismissRecommendation(id)`. The mutator (a) sets the recommendation's `feedback` to `"dismissed"` AND (b) appends to `watched` with `feedback: "dismissed"` and `watchedAt: null`. The `null` watchedAt is the data tell that the user never watched it. The `"dismissed"` feedback in `watched` is exclusion-only: SKILL.md keeps the show out of future picks but does not extract any positive, negative, or ambiguous signals from it. Atomic.
